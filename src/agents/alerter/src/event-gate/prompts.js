...
// Phase 44 Plan-04 Task 4.2: Haiku 4.5 classifier prompts.
//
// SYSTEM_PROMPT is intentionally large (> 20,000 chars) as a conservative
// proxy for the ≥ 4,096-token cache threshold on Haiku 4.5
// (RESEARCH Pitfall 1). The real ship-gate is `usage.cache_creation_input_tokens > 0`
// asserted by the live-fire test in Task 4.5.
//
// W10 holdout: HOLDOUT_ROW_IDS reserves 10 capture ids from
// .planning/phases/44-event-gate-durable-signal-outbound-tenant-aware/44-hand-classified-100.jsonl
// for the live-fire smoke. Those rows' literal text MUST NOT appear in any
// few-shot example below — keeping the live-fire honest.

const HOLDOUT_ROW_IDS = [
  // soft-obs (7 rows, all gray-zone, none of their text appears in few-shot below)
  ...
  '01KRGY9PKT54ZTMRRFPEFV8ARQ', // "Not fruiting chamber but the greenhouse, block unknown"
  '01KRGNCZCRZ2Z14W8DHWGXJYT3', // "Timestamp, just now. Redt, leave blank"
  // UX-meta (3 rows, gray edges, no rule fast-path)
  '01KRQ0RTNV3CE5YV6G299PVKN1', // "Copiado, gracias..." (long compliment+complaint)
...
// Structure: role definition + task contract + tool schema + the six D-20 class
// tags (rubric reproduced from 44-01-CLASSIFICATION-RUBRIC.md) + decision
// procedure + extensive few-shot examples drawn ONLY from the 90 non-holdout
// rows of 44-hand-classified-100.jsonl + a closing reminder.
// ---------------------------------------------------------------------------
