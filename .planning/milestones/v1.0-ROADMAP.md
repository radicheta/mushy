...
**Goal:** Proactive alerts when things go wrong — sensor failures, humidity/CO2 out of range, Pi offline, actuator stuck. Signal bot for mobile notifications (free, simple, works on farm). Alert deduplication so you don't get spammed. Alert history in TimescaleDB.
...
**Builds from scratch:** Threshold engine, alert state machine, Signal integration, alert persistence.
**Requirements:** TBD
**Plans:** 0 plans
...
**Foundation:** fc_camera.py captures 640x480 JPEG at 1 FPS. Bridge stores snapshots to date-organized dirs. MJPEG stream and OpenMCT camera view both working. ~96 snapshots/day at 15-min intervals. Alerts integration via Phase 999.3 (Signal).
