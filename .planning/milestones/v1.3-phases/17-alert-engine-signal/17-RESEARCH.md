...
1. **Networking topology for alerter container.** (A7 above.)
   ...
   - RESOLVED: Plan 01 Wave 0 runs an explicit docker probe that launches a compose-network container with `extra_hosts: ["host.docker.internal:host-gateway"]` and curls the host-mode bridge's `/health`. On PASS, Plans 03/05 use `ws://host.docker.internal:8081`. On FAIL, Plan 01 writes `17-NETWORKING-FALLBACK.md` and Plans 03/05 pivot to host-mode alerter + shared compose network for signal-cli. The fork is resolved at Wave 0 boundary, not at plan-write time.
...
2. **Should the alerter send a "startup" Signal on first boot after a fresh deploy?**
   ...
   - RESOLVED: DEFERRED out of Phase 17. Rationale: the daily heartbeat (§4, fires within 24h of deploy at `ALERT_HEARTBEAT_HOUR`) already provides scheduled liveness attestation, and Plan 05's human-attested smoke test (forced PROBLEM+RECOVERY on deploy day) supplies immediate end-to-end confirmation. A redundant first-boot "hello" ping would add deploy-time noise during development without improving the initial-liveness guarantee. Plans 01-05 do NOT implement a first-boot Signal. Revisit in Phase 20 if operators report they want a deploy-time heartbeat before the morning one lands.

3. **Does the alerter need to emit a ROS topic or Timescale row when it sends a Signal?**
   - RESOLVED: NO for Phase 17. Not required by ALRT-01..08. Out of scope.
   - Logs in stdout are enough; `docker logs alerter` is the audit trail. Deferred candidate for Phase 20 Timescale `alerts` table.

4. **Heartbeat format: should it include "flag it" or "snooze all 24h" hints?**
   - RESOLVED: NO for Phase 17. Keep heartbeat minimal (see §5). Adding hints inflates the message and might confuse. If the farmer asks for it in live usage, add in Phase 20.
