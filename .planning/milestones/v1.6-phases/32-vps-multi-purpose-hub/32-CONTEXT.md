...
| D-10 | **Beta-tester scope (refined 2026-05-10 mid-session):** **farmer #1 on Android phone** (not zoy). Success criterion: farmer #1 can access **Mission Control** via the VPS — she opens `http://10.66.0.12:8080/` on her Android browser through the WireGuard tunnel and sees the live OpenMCT dashboard. Implies: (a) WireGuard for Android app installed; (b) hub IP-forwarding enabled (T2); (c) elder-plops peer reachable on :8080 from her phone via hub; (d) bridge CORS_ORIGIN may need to include `http://10.66.0.12:8080` so MC's API calls don't get blocked. Full-mesh access to fc1 still deferred. | DECISION-6 + operator refinement 2026-05-10 |
