...
**Spec:** `.planning/phases/64.1-extraction-write-path/64.1-DESIGN.md`
...
`_handle_strain_intercept` precedent. Task 8b.

**Completeness criterion for D-3, added 2026-08-17 after the whole-branch
review.** Routing the reply is only half the fix. The reply must produce a
*correct* farmer-facing response. The original wording of this divergence said
only "give it a caller", and the branch duly gave it one — which then dispatched
the stale ask-back text back at the farmer, so answering the question returned
the same question while `needs_input` was quietly cleared. That defect was a
direct consequence of this section being underspecified, not of sloppy
execution. Any divergence that makes an unreachable path reachable must state
what the farmer is supposed to receive at the end of it.

**D-4 — A small multi-entry capture cannot give one confirm prompt per entry.**
Discovered mid-phase by Task 6's implementer, ruled on then, and folded into
this list afterwards so the plan matches what shipped. Phase 53 BACK-02 intended
that a capture yielding a handful of high-confidence drafts produces N
independent confirm prompts. It cannot: the FSM's clean branch always returns
`awaiting_farmer`, and `idx_signal_draft_in_flight_per_sender` permits exactly
one such row per sender, so every sibling insert fails. Node hits the same wall
and silently drops the siblings. Fixing it properly needs a schema change to an
index shared with the live Node agent against the production database — out of
scope here. Fix: all N drafts persist; the FIRST keeps the slot and is prompted;
subsequent would-be-`awaiting_farmer` drafts land in `needs_review` with
`needs_review_reason='multi_confirm_slot_taken'`, deterministically first-wins
rather than last-wins-by-iteration-order. Drafts that reach `needs_review` on
their own merits keep their own reason. Task 6. Tracked as MUSHY-80.
...
- Create: `.planning/phases/64.1-extraction-write-path/64.1-VERIFICATION.md`
...
Expected: PASS, with the preview printed. **Save the full output** to `.planning/phases/64.1-extraction-write-path/64.1-VERIFICATION.md` — never overwrite paid-API results, and the preview text is the artifact worth keeping.
...
```bash
git add src/farm-agent/tests/test_extraction_write_path_live_fire.py src/farm-agent/pyproject.toml .planning/phases/64.1-extraction-write-path/64.1-VERIFICATION.md
...
**Divergence coverage.** D-1 spans Tasks 2 (FSM emits the tag), 3 (`build_confirm_prompt`), 4 (dispatcher sends it), 5 and 6 (pipeline builds the preview), and 10 (asserted end to end). D-2 is Task 8. D-3 is Task 8b, with an explicit escape hatch if it proves not to be containable. D-4 is Task 6, discovered during execution rather than planned.

## Post-execution addendum (2026-08-17)

Two tasks were added mid-phase and one criterion was found missing. Recorded here so this document matches what actually shipped:

- **Task 8c** was added after the discovery that `route_confirm_reply` had no caller anywhere in `farm_agent/` — a second dead seam of the same class as MUSHY-76, one step further down the pipeline. Without it the agent would have created drafts, prompted the farmer, ignored their YES, and expired every one. Scope extension approved by the owner.
- **D-4** was discovered by Task 6's implementer and is now documented above.
- **The D-3 completeness criterion** above was missing from the original text, and its absence produced a real farmer-facing defect that only the whole-branch review caught.

The pattern worth carrying forward: every task in this plan failed its first review on a test that could not fail, and all three Criticals in the final review were seams where a unit test on each side asserted the shape it expected while the two sides disagreed. Unit tests did not catch a single one of those. The tests that did catch things were the ones that observed a seam — a real row in a real database, a payload as actually dispatched, a body as actually sent.
