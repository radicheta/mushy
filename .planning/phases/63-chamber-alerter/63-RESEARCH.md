...
> **⚠ CORRECTED 2026-07-25.** An earlier version of this pitfall stated that BOTH the
> `pi_liveness` and `tick` handlers build `piCfg = {...effective, oobN:1, oobWindowMin:0}`, and
> advised extracting one helper "used consistently at all 6 call sites". That is wrong against
> the source: `state.js:580` (tick/pi) passes plain `effective`. Following the old advice makes
> the two pi paths consistent, which CHANGES SC1 timing and fails the Phase-64 parity gate. The
> matrix below was read off `state.js` line by line.

**What goes wrong:** the fast-fire override (`oobN: 1, oobWindowMin: 0`) is applied **per call
site, not uniformly**, and the base config being spread differs too (`effective` vs raw env
`config`). It is intentional where it appears (Phase 46 D-10: the 3-min hard threshold in
`isPiOffline` IS the flap protection; layering the generic 5-count/8-min debounce on top would
push FIRING to T0+~11min) — but it is NOT applied everywhere you would expect.

Every `driveAlertType` call site, verified 2026-07-25:
...
Three consequences fall out of this, all of which the port must reproduce:

1. **pi fast-fires on `pi_liveness` but NOT on `tick`** (547 vs 580). An outage the 10s health
   poll reports fires within ~1 eval tick; the same outage seen only by the periodic tick goes
   through the full oob_n=5 / oob_window_min=3 debounce. This directly shapes SC1 timing.
2. **sensor's FIRING path fast-fires but its RECOVERY path does not** (354 vs 359).
   `driveAlertType` counts recovery with `config.oobN`, so a sensor alert fires on one error
   event and needs five clean events to clear.
3. **sensor/sht30/scd41 never see Tier B/C overrides at all** — every one of their call sites
   spreads raw `config`, not `effective`. `isSensorSilent` at 621 likewise receives raw
   `config`, so a Tier C `sensorOfflineMin` override does not reach the per-sensor watchdog,
   despite D-01's "detectors consume the effective config" framing.

**Why it happens:** the override is an inline object spread at each site rather than a named
constant, so the eye reads "they all do this" and moves on.

**How to avoid:** model it as TWO independent axes per call site, not one helper. A
`_fast_fire(cfg)` helper may return a copy with `oob_n=1, oob_window_min=0`, but the BASE
(`effective` vs raw env config) is chosen separately and deliberately at each site against the
table above. Do not "make it consistent."

**Warning signs:** parity replay shows pi alerts firing systematically EARLIER than Node on the
tick path, sensor alerts recovering after one clean sample instead of five, or sht30/scd41
honouring a runtime `sensorOfflineMin` override that Node ignores.

**Disposition:** consequences 1-3 are reproduced and pinned by `*_parity_quirk_*` tests in
Plan 07, and filed as Phase-64 intentional-delta CANDIDATES in `63-07-SUMMARY.md`. They are not
fixed in Phase 63 — the delta list is fixed at the TZ fix, quote-ts coercion and fmtNum.
...
| CHM-01 | `is_pi_offline` fc1-dark hard 3-min threshold, independent of `pi_offline_min` (Pitfall 2) | unit | `uv run pytest tests/chamber/test_rules.py::test_pi_offline_fc1_dark_hardcoded_3min_ignores_config -x` | ❌ Wave 0 |
| CHM-01 | `is_humidifier_stuck` offline-blindness 3-gate order (Pitfall 5) | unit | `uv run pytest tests/chamber/test_rules.py -k "humidifier_stuck_gate" -x` | ❌ Wave 0 |
...
| CHM-01 | `resolve_effective_config` full Tier A/B/C + freshness matrix (fresh/stale/cold × mode-present/absent) | unit | `uv run pytest tests/chamber/test_state.py -k "tier_a or tier_b or tier_c" -x` | ❌ Wave 0 |
| CHM-01 | `drive_alert_type` fast-fire override (`oob_n=1, oob_window_min=0`) applied to `pi`/`sensor`/`sht30`/`scd41` but NOT `rh`/`humidifier` (Pitfall 3) | unit | `uv run pytest tests/chamber/test_state.py -k "fast_fire or parity_quirk_tick" -x` | ❌ Wave 0 |
...
| CHM-01 | Heartbeat "defer if summary empty" retry semantics (Pitfall 10) | unit | `uv run pytest tests/chamber/test_heartbeat.py::test_defers_on_empty_summary_then_fires -x` | ❌ Wave 0 |
| CHM-01 | WS reconnect/backoff schedule (1s→30s doubling, matches `bridge-client.js`) | unit (fake/mock WS) | `uv run pytest tests/chamber/test_ws_client.py::test_backoff_doubles_then_caps_at_30s -x` | ❌ Wave 0 |
...
| CHM-01 (D-05) | Composite dispatch routes snooze/mute text to chamber, all else to capture pipeline | integration | `uv run pytest tests/chamber/test_service_wiring.py -k "routes_to_chamber or falls_through_to_the_pipeline" -x` | ❌ Wave 0 |
