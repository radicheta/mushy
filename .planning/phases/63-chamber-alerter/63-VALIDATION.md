...
| 63-02-02 | 02 | 1 | CHM-01/02 | T-63-SC/TZ | deps installed; Montevideo zone resolves to UTC-3 | unit | `cd src/farm-agent && uv run pytest tests/chamber/test_deps.py -v` | ❌ W0 | ⬜ pending |
...
| 63-06-01 | 06 | 2 | CHM-01 (SC1 timing) | T-63-10 | backoff 1s→30s doubling + health parsing, as pure fns | unit | `cd src/farm-agent && uv run pytest tests/chamber/test_ws_client.py -x` | ❌ W0 | ⬜ pending |
| 63-06-02 | 06 | 2 | CHM-01 | T-63-10/11 | reconnect loop, fail-open health poll, injected http | unit | `cd src/farm-agent && uv run pytest tests/chamber/test_ws_client.py -x` | ❌ W0 | ⬜ pending |
...
| 63-08-01 | 08 | 4 | CHM-01 (SC1/D-05/D-06) | T-63-14/15/17 | service composes; SC1 automated leg; composite routing | integration | `cd src/farm-agent && uv run pytest tests/chamber/test_service_wiring.py -x` | ❌ W0 | ⬜ pending |
| 63-08-02 | 08 | 4 | CHM-01 (D-05) | T-63-14/16 | one SignalClient + one ReceiveLoop, rate-cap hook, shutdown symmetry | integration | `cd src/farm-agent && uv run pytest tests/chamber/test_service_wiring.py tests/test_boot.py -x` | ❌ W0 | ⬜ pending |
...
- [ ] Corrected `src/farm-agent/.lint-imports` `source_modules` (Plan 01, Pitfall 7) + `test_foray_seam.py` `FORAY_PACKAGES` list, plus the on-disk drift guard (Plan 01, Pitfall 8)
- [ ] `tests/test_import_linter_contract.py` — new, wires lint-imports into pytest for the first time (Plan 01, D-00)
- [ ] `uv add websockets tzdata` + legitimacy gate + `tests/chamber/test_deps.py` zone-resolution smoke (Plan 02)
...
- [ ] Update `tests/test_tenancy.py` + `tests/test_signal_ratecap.py` for the D-03 field move (Plan 03) — note the blast radius is **4** rate-cap tests, not 1; the fix is in `_make_client`, not per-test
