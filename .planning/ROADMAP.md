...
- **Phase 999.14: Index camera snapshots in Timescale for MC history scrubbing** — Bridge writes JPEGs to `/data/snapshots/<camera>/<date>/` every 15min (`src/mission-control/bridge/src/index.js:381`) but nothing indexes them. Add a `snapshots` table (camera_id, captured_at, file_path, bytes) written alongside each `saveSnapshot()` call; expose a timeline scrubber in Mission Control so the farmer can browse chamber history without SSH. Context: surfaced 2026-04-17 while debugging an fc_camera idle-mode stall — farmer had no easy way to find the last good frame. Composes with 999.5 (time-lapse) and 999.11 (farmer app "story view").
