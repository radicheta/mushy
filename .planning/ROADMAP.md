...
**Success Criteria** (what must be TRUE):
  ...
  3. `uv run pytest tests/` passes; a structural diff of `model.model_json_schema()` output versus the Node `SUBMISSION_JSON_SCHEMA` shows zero discrepancies (`extra='forbid'` present on every nested model).
...
- [x] 56-01-PLAN.md — uv project scaffold + Dockerfile + conftest + committed Node JSON-Schema fixture (Wave 1, FND-01)
- [x] 56-02-PLAN.md — tenancy/TenantConfig layered loader, secrets env-only, traversal guard (Wave 2, FND-02)
- [x] 56-03-PLAN.md — Foray seam CI gate (grep + import-linter config) (Wave 2, FND-05)
- [x] 56-04-PLAN.md — pydantic v2 schemas + zod-parity gate (spike then full) (Wave 2, FND-04)
- [x] 56-05-PLAN.md — persistence psycopg3 async pool + additive idempotent migrations (Wave 3, FND-03)
- [x] 56-06-PLAN.md — boot.py daemon wiring + alerter-py compose service (Wave 4, FND-01)

### Phase 57: Signal I/O

**Goal**: The Python stack can send and receive Signal messages through the live `signal-cli-rest-api` HTTP container (REST->httpx 1:1 port per D-01/research; the SIG-01 "JSON-RPC socket" phrasing is superseded -- switching transport is a hard blocker), with durable fail-open outbound persistence, asyncio.Lock rate-cap, correct multi-farmer routing, and the intent-agnostic wire-level quote primitive that folds in the Phase-50 fixes.
**Depends on**: Phase 56
...
- [x] 57-01-PLAN.md — deps (httpx+respx) + TenantConfig (api_url/additional_senders/mask_number) + never-throw outbound_repo (SIG-02 foundation)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 57-02-PLAN.md — SignalClient: send/receive/fetch_attachment/accounts via httpx, quote primitive, group translation, asyncio.Lock rate-cap, fail-open persist (SIG-01/02/04; SC#1-4)
- [x] 57-03-PLAN.md — router.py + receive_loop.py: whitelist, DM-vs-group, triggers, (unassigned) resolution, sequential dispatch (SIG-03; SC#5)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 57-04-PLAN.md — live-fire self-send harness: SC#1 bigint round-trip + SC#3 native quote bubble (autonomous: false) — PASS 2026-06-21; caught + fixed flat-vs-nested quote shape-drift on 0.200-dev
...
- [x] 58-04-PLAN.md — SC#1 live-fire: real voice note -> non-null transcript (autonomous: false, BLOCKED on D-07 Whisper health + A5 mount)
...
- **Phase 999.33: Digital twin / simulation of FC chamber for offline control development** [MUSHY-52] — **SHAPE (a) BUILT 2026-08-09** on branch `feat/chamber-sim-duty-shaping`: `fc_core.sim` (FOPDT chamber model, PWM/relay simulator with pipe-transit loss and wear counters, closed-loop replay harness) plus `fc_core.control_kernel`, a pure extraction of the live band/feather projection so sim and controller cannot drift. Reproduces the 2026-08-08 limit cycle and was immediately load-bearing: it found the root cause of the ~2 h humidity cycle and killed two plausible-but-wrong fixes before either touched the chamber (see MUSHY-56). **Correction to the numbers below:** the first-principles chamber model this entry specified (`air_mass_kg=7.0`, `mister_rate_g_per_min=6.0`) is wrong by ~40x — it predicts ~950 pts/h at full duty against a measured 22.5 pts/h, because the wet substrate is a moisture buffer orders of magnitude larger than the air volume. The shipped model is fitted to measured telemetry instead. Known fidelity limits are documented in `test_replay_fidelity.py` and `.planning/notes/2026-08-09-pid-limit-cycle/sim-results.md`. Original entry follows. **PRIORITY BUMP 2026-06-22 (farmer-requested): "the more we deal with this the more we want the digital twin."** Today's calibration session was a near-textbook case for it: we designed + shipped the humidifier feather + Mode C reserve (commit `30534ff` on `fc1/prod`) and judged the decay-tau experiment a failure — every step blind against the live chamber, gated on a real power-cut/outage recovery, winter cooling driving a falling-temp regime, and farmer presence. We shipped a real setpoint-anchoring bug LIVE (the midpoint anchor silently raised the operating RH from floor-0.945 to mid-0.96) and only caught it hours later via the farmer reading the MC target line — a divergence a twin's replay would have flagged in seconds. And we now have a stack of experiments ALL blocked behind live-chamber exposure: the slew limiter, temperature feedforward, halving decay_tau, and re-anchoring the feather on band_low. Shape (a) (the lightweight Python ODE) could validate every one of them offline against this week's Timescale trace — including today's confirmed falling-temp passive-RH-rise data (abs moisture fell 52→49 while RH held 96 as temp dropped 10.1→9.0). See memory `project_humidifier_feather_deployed_2026_06_22`. **Treat as a near-term ship: do shape (a) before the next live-tuning round.** Original entry: Surfaced 2026-05-04 during PID calibration session 2. Today produced *six* substantive control-design artifacts (Kp tuning, capacity envelope, two flavors of overshoot, integrator-driven limit cycle, derivative-filter bug, feedforward case) — all of them required real chamber + a live grow + ambient temperature swings + farmer presence. Iteration cadence: ~1 idea per real-world day, gated on actual weather. **Strategic ask:** stand up a digital twin so we can iterate control logic at 100× real-time, validate against historical data, and stop blocking on live-grow exposure for tuning experiments. **What we have for system ID** (calibrated empirically today, 2026-05-04): chamber V = 5.76 m³ (2.4 × 1.2 × 2.0 m grow tent), m_air ≈ 7 kg, mister output M ≈ 6 g/min, dead time θ ≈ 50 s (impulse), chamber τ ≈ 10 min (1st-order), 19-min sustained-disturbance lag (suggests 2nd-order dynamics under perturbation), leakage L scales 3.5× from cold-dry to warm-wet (matches saturation-pressure-differential physics). All in `docs/pid_calibration_notes.md` "Chamber Dynamics" section. Plant model is approximately: `dRH/dt = (M·duty − L·(P_v_in − P_v_out)) / (m_air × dRH/dw_at_T) − dT/dt × dRH/dT_at_constant_water` — a 1st-order ODE with temperature as an exogenous disturbance. **Two implementation shapes:**
...
- **Phase 999.34: SHT30 heater cycle as a wet-membrane plausibility check** [MUSHY-55] — Surfaced 2026-05-04 evening; premise substantially revised 2026-08-08 (see "Re-test" below). FC-1 sits at 94–96% RH effectively 24/7, the stress regime where membrane condensation makes the RH reading drift positive (sensor reads near-saturated even when the air isn't). The heater is **already accessible** — `adafruit_sht31d` exposes it as `self.sht.heater = True/False`, no driver work needed. Heater raises *sensor* T by a few °C; chamber bulk T is unaffected, but the sensor's own T+RH readings are corrupted during heating plus a recovery tail before they re-equilibrate.

  **PREMISE CORRECTION (2026-08-08).** This entry previously claimed "SHT30 datasheet recommends running the on-die heater periodically when the sensor is exposed to sustained high humidity." **That is not in the SHT3x datasheet.** Section 4.10 says the opposite in scope: *"The SHT3x is equipped with an internal heater, which is meant for plausibility checking only. The temperature increase achieved by the heater depends on various parameters and lies in the range of a few degrees centigrade."* Condensation/spray-water removal and the <10% duty-cycle guidance belong to the **SHT4x** datasheet, a different part we do not have. The SHT3x datasheet states no duty-cycle limit and makes no condensation-removal claim at all. Also corrected: heater power is **P_Heater 3.6–33 mW depending on supply voltage** (~12 mW at 3.3 V), not "3.6 mA at 3.3 V" as previously written. Consequence: reframe this phase from *therapeutic* (clear the condensation) to *diagnostic* (detect a wet membrane). See open question 7.

  **Re-test 2026-08-08 23:33–23:42 UYT (farmer-requested, fc1 live, fc-core running throughout).** Four manual activations. Raw data + full writeup: `.planning/notes/2026-08-08-sht30-heater-pulse/`. Baseline was T=4.87 °C / RH=89.3% — note this is *not* the 94–96% condensing regime the phase targets, so it is a weak test of the condensation hypothesis and a strong test of the mechanism.

  1. **No exclusive bus access required.** The pulse was previously assumed to need `fc-core` stopped so nothing else drove the I2C bus. It does not. `fc_sensors` builds its `SHT31D` once (`fc_sensors.py:35`) and thereafter only reads `.temperature` / `.relative_humidity`; it never issues a soft reset, never re-inits, never touches the heater bit. A second process can hold the heater on across dozens of concurrent `fc_sensors` reads. **This removes the telemetry-gap cost from the design** — the pulse can run in-process with sensors publishing continuously.

  2. **`adafruit_sht31d`'s `.heater` getter is unreliable — do not trust it.** It reported `False` at t+3 s while the heater was demonstrably still on (status register `0x2000` set, temperature climbing monotonically). Read the status register directly: `bool(sensor.status & 0x2000)`. Datasheet Table: bit 13 is heater status. An implementation that polls `.heater` to confirm the pulse will silently believe the heater failed. **This cost ~40 min of misdiagnosis tonight** — it produced a confident, wrong "bus contention is stomping the heater bit" conclusion.

  3. **+0.73 °C at 3 s reproduces exactly.** 2026-05-04 measured +0.73 °C at 3 s from a 13.85 °C baseline; 2026-08-08 measured **+0.730 °C at 3.1 s** from a 4.87 °C baseline. Two ambients ~9 °C apart, same rise. Full curve at 1 s resolution: +0.452 / +0.617 / +0.730 / +0.786 / +0.898 / +0.973 / +1.029 / +1.042 / +1.085 / +1.125 / +1.181 / **+1.237 at 12.2 s**. Rise is fast then strongly sublinear; the marginal °C per second past ~6 s is small. 12 s is comfortably inside the datasheet's "a few degrees centigrade."

  4. **Dew-point depression is the metric that matters, not ΔT.** At the 2026-08-08 baseline, dew point was 3.26 °C, so depression was only **1.61 °C**. The pulse raised it to **2.34 °C at 3 s** and **2.85 °C at 12 s** — nearly doubling the margin. Absolute ΔT of ~1 °C reads as unimpressive and is the wrong frame; state the depression in any future writeup.

  5. **A dry membrane has a signature, and we measured it.** If the membrane is dry, heating at fixed vapor pressure predicts the RH curve exactly. Predicted RH at the +5 s recovery point (T=5.31 °C) was **86.6%**; measured **87.08%**. Half a point apart. A *wet* membrane would deviate the other way — evaporating condensate pins local vapor pressure up, so RH stays much nearer saturation than the dry-air prediction. **This is the actual deliverable of the phase**: the residual between predicted-dry and measured RH is a direct wet/dry discriminator, and it is exactly the "plausibility check" the datasheet describes. Corroborating evidence the sensor was dry: SHT30 read 89.5% against SCD41's 91.6%, i.e. 2 points *below* — condensation drift reads *high*.

  6. **Energy budget bounds what the heater can ever evaporate.** ~12 mW at 3.3 V over 3 s is ~36 mJ; at ~2490 J/g latent heat that is **~15 µg of water**, roughly a 3 µm film over a few mm² of membrane. 12 s buys ~58 µg. And that is the optimistic bound — most heater energy warms the die and conducts away rather than going into latent heat. Adequate against a thin film, useless against an actual droplet. Reinforces diagnostic-over-therapeutic.

  7. **The PID spike is real but much milder than 2026-05-04.** Duty went **0.34 → 0.46** (vs 0 → 0.85 in May) chasing an RH dip to 87.08%. RH and T were fully back to baseline within ~60 s of heater-off; duty decayed on its own. 999.32's derivative filter having shipped since May is the likely reason. The controller guard is still wanted, but "heater without guard is net-negative" (below) now overstates the risk at fruiting-mode duties.

  8. **One activation produced no heat at all and remains unexplained.** 23:33:19 UYT, same 3 s command, flat telemetry (4.716–4.759 °C throughout, zero signature in `fc.temperature`). The 23:38 and both 12 s runs all heated normally. Best guess is a lost I2C write, consistent with the `OSError: [Errno 5]` that killed that same script ~3 min later — but that is a guess, not a diagnosis. **Any implementation must verify the pulse actually fired** (status bit + a minimum expected ΔT) rather than assume the write landed.
  ...
  **Open design questions for planning** (each with a recommended default; resolve during discuss-phase):
  ...
  2. **Pulse duration + recovery:** 1 s heater on, then ignore sensor readings post-heat. ~~Datasheet ranges 1–3 s pulses.~~ *(2026-08-08: the SHT3x datasheet specifies no pulse-duration range and no duty-cycle limit — that guidance was imported from the SHT4x part. 1–3 s was our own convention, not a spec.)* **Updated recommendation: 3 s.** It is the only duration measured twice at two ambients with an identical +0.73 °C result, and the 1 s-resolution curve shows the marginal gain past ~6 s is small while the recovery cost keeps growing. **Live test 2026-05-04 22:02 UTC measured numbers** with 3 s pulse: sensor T peak +0.73 °C (14.58 vs 13.85 baseline, well within datasheet 0.5–1.5 °C); RH dipped -0.55 % (95.83 vs 96.39); **T recovery to baseline: ~60 s; RH recovery to baseline trend: ~150 s (2.5 min)**. RH recovery is the binding constraint — *not* 30 s as initially specified. Default updated: **1 s pulse, hold-last-duty for 180 s** (3 min), or condition-driven release (T within 0.05 °C of pre-pulse and slope ~0). 1 s pulse on a clean (uncondensed) sensor likely shortens RH recovery somewhat — re-measure after first nightly pulse on production sensor.

  3. **Controller behavior during cycle:** hold last duty (recommended — chamber inertia tolerates 3 min of open-loop fine) vs go to safe-state OFF (rejected — would cause unnecessary RH dip) vs continue PID against last-good-reading (rejected — controller would drift on stale data). Default: hold-last-duty for the full heater + recovery window. **Live test confirmed this is non-optional**: the same 22:02 UTC pulse, with no controller guard in place, caused the PID to spike duty from 0 → ~**0.85** for ~60 s in response to the synthetic RH dip. In active control during a daytime warm-up regime, that spurious 0.85 spike for ~1 min would inject ~5 g of extra water and produce a ~0.6 % real RH overshoot — directly re-triggering the integrator-driven cycle the day's tuning work was meant to fix. **Heater feature without controller guard is net-negative**; both must ship together. (Bug 999.32 unfiltered derivative compounds this — D-term saw the synthetic falling-RH and slammed duty up; once 999.32 ships the spurious spike will be smaller but still present from P+I terms.) **Softened 2026-08-08:** 999.32 has since shipped, and the re-test measured 0.34 → 0.46 rather than 0 → 0.85, recovering unaided within ~60 s. The prediction that the filter would shrink the spike held. Guard still recommended, but it is no longer a hard ship-blocker at fruiting duties — reassess before treating it as one.

  7. **NEW (2026-08-08) — diagnostic or therapeutic? Pick one, because they want different designs.** The datasheet says the SHT3x heater is for plausibility checking only, and the energy budget (finding 6) says it cannot evaporate more than a thin film. If the goal is **diagnostic** (detect a wet membrane), the design is: pulse, compute predicted-dry RH from the measured ΔT at fixed vapor pressure, and alarm on the residual (finding 5) — cadence can be infrequent, the pulse can be short, and no claim about "clearing" anything is made. If the goal is **therapeutic** (actually dry the membrane), we need evidence a 3 s / 15 µg pulse changes anything, and tonight provided none because the sensor was already dry. **Recommended default: build the diagnostic.** It is what the hardware is specified for, it is testable in one pulse instead of a 30-day soak, and it directly serves the real objective — knowing whether to trust the RH reading. The therapeutic version can be reconsidered if the diagnostic ever reports a persistently wet membrane.
  ...
  **Acceptance additions 2026-08-08:** (7) the pulse is verified to have actually fired — status bit `0x2000` read directly (never `.heater`) **and** observed ΔT ≥ 0.3 °C at 3 s; a pulse that fails either check is logged as `heater_pulse_failed`, not silently ignored (see finding 8); (8) `fc_sensors` keeps publishing uninterrupted across the entire heater window — no telemetry gap, since finding 1 shows none is needed; (9) the wet/dry residual (measured RH minus predicted-dry RH at the observed ΔT) is computed and published per pulse — this is the phase's actual output under the diagnostic framing in question 7.

  **Still unproven after 2026-08-08:** whether FC-1's SHT30 ever *is* wet. Every pulse so far landed on a demonstrably dry membrane. The next useful test is a pulse taken when the chamber is back at 94%+ **and** SHT30 reads *above* SCD41 (the condensation-drift direction) — that is the first datapoint that could actually support the original premise. Until then this phase is a solution looking for its problem, and should be sequenced accordingly.

  **Filed 2026-05-04 evening** — adjacent to the calibration work but architecturally orthogonal to the PID retune. Could ship independently of the Saturday filter+Kd retune work. **Revised 2026-08-08** after farmer-requested live re-test.
