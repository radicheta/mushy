...
- **Phase 999.22: BUG — Alerter RH target/band must read from ROS, not env** — Surfaced 2026-04-28 when farmer changed `target_humidity` 0.90→0.94 + `humidity_tolerance` 0.01→0.015 via `ros2 param set` on `/fc_controller`. Controller picked it up live, but the alerter kept firing OOB pages against its own hardcoded copy (`ALERT_RH_TARGET=90`, `ALERT_RH_BAND=3` from `docker-compose.override.yml` / `.env`). Two-source-of-truth bug: every humidity tweak now requires editing the controller param AND the alerter env AND restarting the alerter container. Farmer (correctly) called this out — fixing it downstream by syncing the env each time is the wrong solution. **Fix direction:** alerter should read target + band from the controller. Two shapes: (a) subscribe to a ROS param-broadcast topic the controller publishes (cleanest, but requires controller-side work to publish); (b) alerter polls the bridge for `/fc_controller`'s param values via a small bridge endpoint (`/api/fc1/control_params`) — the bridge already speaks ROS to the controller. Touches `src/agents/alerter/src/config.js` (drop `ALERT_RH_TARGET/BAND` env defaults, fetch dynamically), `src/agents/alerter/src/rules.js`/`message.js` (re-evaluate band on each tick instead of capture once at boot), possibly `src/mission-control/bridge/src/index.js` (new endpoint or websocket message type). **Interim state:** `.env` on elder-plops carries `ALERT_RH_TARGET=94` `ALERT_RH_BAND=3` matching the live controller — don't forget to update both if the farmer tweaks again before this lands. Acceptance: `ros2 param set /fc_controller target_humidity X` is reflected in the next alerter evaluation cycle without any container/env/restart action. **Composes with 999.23** — the dynamic-target work means the alerter's "current target" will change *over time within a single grow* (ramps, scheduled day/night cycles, fruiting-stage transitions), so reading-from-controller isn't an optimization, it's a correctness requirement. Whatever fix shape lands here should expose the *current effective target+band at evaluation time*, not a static-at-boot snapshot.
