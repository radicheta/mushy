...
- **Phase 999.6: Multi-chamber scaling** — deploy 3-5 additional chambers on Pi 3B (1GB RAM) hardware using a single-node `chamber_lite` ROS2 package. Architectural decisions from 2026-04-13 design discussion:
    - **Hardware:** Raspberry Pi 3B (user has several on hand). 1GB RAM is sufficient for one combined ROS2 node (~200MB with OS), headless, Ubuntu Server 24.04 arm64, `ros-jazzy-ros-base` (not desktop).
    - **`chamber_lite` package:** single ROS2 Python node combining sensor reading + bang-bang control + topic publishing. Parameterized by `chamber_id` — each chamber config YAML sets its ID, sensor address, target humidity, pin assignments. No camera, no display, no telemetry node, no fan PWM (logs don't need it). Publishes `/{chamber_id}/humidity`, `/{chamber_id}/temperature`, `/{chamber_id}/actuators/humidifier` using the same message types as fc1.
    - **Bridge changes:** dynamic subscription loop driven by `CHAMBERS` env var (e.g. `CHAMBERS=fc1,log1,log2,log3`). Bridge iterates chamber IDs, subscribes to each chamber's topics, broadcasts via WebSocket and inserts into TimescaleDB with topic prefix `{id}.humidity` etc. `ALLOWED_TOPICS` generated from the chamber list. Adding a new chamber = add its ID to the env var and restart the bridge. No code changes.
    - **fc_core refactor:** move fc1's hardcoded `fc1/` topic prefix to a `chamber_id` parameter so fc1 uses the same namespace convention. fc_core stays as the full-featured package (camera, display, CO2, fan PWM); `chamber_lite` is the stripped variant for simple chambers.
    - **First target:** log chamber (outdoor shiitake logs), humidity-only control, SHT30 on I2C, SSR for mister. WiFi via farm 4G MiFi (same as fc1).
    - **Pi Zero W satellites (I/O nodes):** Pi Zeros as dumb sensor/actuator endpoints managed by a Pi 3B zone controller. Each Zero runs a minimal Python script (no ROS2) — reads an SHT30 over I2C, drives a MOSFET that opens a solenoid valve for irrigation, reports readings and accepts commands over WiFi (TCP/UDP/HTTP) from the Pi 3B's ROS2 node. All control logic (humidity thresholds, irrigation timing, dwell) lives on the Pi 3B. The Zero is a ~$10 remote GPIO adapter. Use case: log stacks spread across an outdoor area where running sensor cables is impractical — each zone gets its own Zero + sensor + solenoid valve. A Pi 3B can manage multiple Zeros, each publishing under the same chamber namespace (e.g. `log1/zone_a/humidity`, `log1/zone_b/humidity`). USB OTG gadget mode is an alternative for short distances (<5m) — Zero appears as a serial device plugged directly into the Pi 3B, no WiFi needed. **This may ship before the Pi 3B chamber_lite work** — fc1 already runs ROS2 and could act as the zone controller for a first satellite, validating the pattern before standing up a separate Pi 3B.
    - **Standalone (no-ROS2) alternative was evaluated and rejected** — HTTP ingest endpoint would work for one chamber but doesn't scale; ROS2 DDS discovery means each new Pi auto-integrates with zero bridge code changes once the dynamic subscription loop is in place. 3-5 chambers tips the balance toward ROS2.
    - Depends on: nothing strictly, but benefits from Phase 12 (subscriber-aware camera pattern is a template for resource-conscious nodes). Composes with 999.3 (alerts for multiple chambers), 999.11 (farmer app multi-chamber view).
