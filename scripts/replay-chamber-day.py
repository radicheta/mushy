...
FREE for 2026-08-08 00:00..2026-08-09 00:00 UTC, starting from the day's
FIRST recorded RH, but is fed the REAL per-second ambient absolute humidity
...
the day step-hold that final hourly sample (documented, not silently done).

=== Output ===
  .planning/phases/999.33-digital-twin-chamber-sim/999.33-08-chamber-day-2026-08-08.csv
      time_utc, rh_recorded, rh_predicted, error, duty_recorded, duty_predicted,
      ambient_ah, temp_c -- full 1 Hz resolution, 86400 rows.
  .planning/phases/999.33-digital-twin-chamber-sim/999.33-08-CHAMBER-DAY.md
      metrics + diagnosis.
"""
import subprocess
import sys
...
def build_grid(paths: dict):
    ...
    rh_raw = pd.read_csv(paths['rh'], header=None, names=['t', 'rh_pct']).sort_values('t')
    temp_raw = pd.read_csv(paths['temp'], header=None, names=['t', 'temp_c']).sort_values('t')
    ...
    relay_raw = pd.read_csv(paths['relay'], header=None, names=['t', 'state']).sort_values('t')
    seed = pd.read_csv(paths['relay_seed'], header=None, names=['t', 'state'])
    seed_row = pd.DataFrame({'t': [float(t0) - 1.0], 'state': [seed['state'].iloc[0]]}) \
        if len(seed) else pd.DataFrame({'t': [], 'state': []})
...
def build_ambient_ah(full_idx) -> np.ndarray:
    """Real per-second ambient absolute humidity from the MUSHY-64 fixture.

    The fixture's coverage ends exactly at 2026-08-08T23:00 (last hourly
    sample of the whole fixture's date range) -- there is no 2026-08-09T00:00
    point to interpolate toward. Times past the last sample step-hold it
    (same treatment AmbientSeries already gives precipitation within its
    covered range), rather than raising, so the final partial hour of the day
    uses the last known real hourly reading instead of extrapolating.
    """
...
def main():
    ...
    print(f'Driving 24h from rh0={rh0:.3f}% temp0={temp0:.3f}C ...', file=sys.stderr)
    ...
    assert len(rh_predicted) == len(grid['full_idx']) == 86400, \
        f'expected 86400 steps, got {len(rh_predicted)}'
    ...
    hourly_gradient = gradient.reshape(24, 3600).mean(axis=1)
    ...
    with open(OUT_CSV, 'w', newline='') as f:
        f.write(
            ...
            'NOT a bucket average. 86400 rows, 1 Hz.\n')
    ...
    print(f'wrote {OUT_CSV} (86400 rows)', file=sys.stderr)
